...
from .gen_denoise import Denoise 
import hydra 
...
class latent_multinomial_diffusion(L.LightningModule):
    def __init__(self, config, VAE_DENSE, multi_criterion,
                 auxiliary_loss_weight=0.0005, adaptive_auxiliary_loss=True):
        super().__init__()
        self.save_hyperparameters(ignore=["VAE_DENSE"])
        self.config = config 
        self.args = config.model
        self.num_classes = self.config.model.num_classes * self.config.model.vq_size
        self.denoise = Denoise(args=self.config.model,  num_class = self.num_classes)
        
        self.num_timesteps = self.config.model.diffusion_steps
    ...
    def sample(self, x):
        '''
        x: multi-categorical voxel (B,X,Y,Z) or whatever your minecraft voxel structure comes in 

        Returns a single sample (X,Y,Z)
        '''
        device = self.log_alpha.device
        self.shape = x.size()[1:]
        
        x = torch.randint(self.config.model.num_classes, size=x.size()).to(device)
        latent = self.VAE_DENSE.encode(x)
        _, _, sparse_ind, latents_shape = self.VAE_DENSE.vector_quantize(latent)
        reshape_size = [latent.size()[0], latent.size()[2], latent.size()[3], latent.size()[4]] #(B,H,W,Z)
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            t = torch.full((self.config.loader.global_batch_size,), i, device=device, dtype=torch.long)
        ...
        vq_latent = self.VAE_DENSE.codebook(vq_ind.view(-1,1), latents_shape)
