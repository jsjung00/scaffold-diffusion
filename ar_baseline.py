...
class ARBaseline(L.LightningModule):
    def __init__(self, config, tokenizer):
        ...
        # define backbone 
        self.backbone = models.dit3d_ar.DITAR(self.config, self.vocab_size)
    ...
    def _subs_parameterization(self, logits, xt):
        ...
        # also ensure that [PAD] tokens are never selected
        logits[..., self.pad_token] += self.neg_infinity
    ...
    def forward(self, x, token_pos, pad_mask):
        """Returns log score.
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            logits = self.backbone(x, token_pos, pad_mask)  # (B,L, Vocab)
    ...
    def _compute_loss(self, batch, prefix):
        ...
        loss = self._loss(batch, occupancy_map)
    ...
    def _get_token_pos_ids(self, x0, occupancy_map):
        ...
        Returns 
            token_pos: (torch.tensor) of shape (B, L, 3) where first pos is zeros and right padded with zeros
            token_ids: (torch.tensor) of shape (B, L) where first token is <BOS> right padded with pad_tokens
        '''
        B = x0.shape[0]
        ...
        for b in range(B):
            ...
            if len(batch_active_indices) > self.config.model.length:
                ...
                bos_pos = torch.zeros(1, 3, dtype=batch_active_indices.dtype,\
                                      device=batch_active_indices.device)

                # right pad with zeros to length L
                num_pad_tokens = self.config.model.length - len(batch_active_indices) - 1
                if num_pad_tokens > 0:
                    pad_zeros = torch.zeros(num_pad_tokens, 3, dtype=batch_active_indices.dtype,\
                                            device=batch_active_indices.device)
                    final_seq = torch.cat([bos_pos, batch_active_indices, pad_zeros], dim=0)
                else:
                    final_seq = torch.cat([bos_pos, batch_active_indices], dim=0)
                
                
                # Truncate in case sequence is fully 1024 and we add one too many with <BOS> 
                if len(final_seq) > self.config.model.length:
                    final_seq = final_seq[:self.config.model.length]
        ...
        for b in range(B):
            ...
            if len(active_token_ids) > self.config.model.length:
                ...
                num_pad_tokens = self.config.model.length - len(active_token_ids) - 1

                bos_tensor = torch.tensor([self.bos_token], dtype=active_token_ids.dtype, 
                ...
                if num_pad_tokens > 0:
                    pad_tokens = torch.full([num_pad_tokens,], fill_value=self.pad_token, dtype=active_token_ids.dtype,
                                            device=active_token_ids.device)
                    final_seq = torch.cat([bos_tensor, active_token_ids, pad_tokens], dim=0)
                else:
                    final_seq = torch.cat([bos_tensor, active_token_ids], dim=0)
                    if len(final_seq) > self.config.model.length:
                        final_seq = final_seq[:self.config.model.length]
    ...
    def _forward_pass_ar(self, token_ids, token_pos, pad_mask):
        '''
        ...
        logits = self.forward(token_ids, token_pos, pad_mask)  # (B, L, Vocab)

        # calculate next token loss
        shift_logits = logits[:, :-1, :].contiguous()
        
        shift_labels = token_ids.clone()
    ...
    def _loss(self, x0, occupancy_map):
        ...
        loss = self._forward_pass_ar(token_ids, token_pos, pad_mask)
        return loss 
