...
pattern = re.compile(
    r'^(\d+(?::\d+)?)\s+.*?\n\((minecraft:[^)]+)\)', re.MULTILINE)

id_to_name = {}
for full_id, resource in pattern.findall(raw):
    base = int(full_id.split(':')[0])      # keep only the part before “:”
    if base < 256 and base not in id_to_name:
        id_to_name[base] = resource

with open("block_id_map.json", "w") as f:
    json.dump(dict(list(id_to_name.items())), f, indent=4)

print(len(id_to_name))          # should be ≤ 256 (usually 254; 36, 253, 254 are unused)
pprint(dict(list(id_to_name.items())[:25]))   # preview first 25 entries
