...
class _DDiTBlock(nn.Module):
    ...
    def forward(self, x, pad_mask, seqlens=None):
        '''
        pad_mask: (B,L) boolean mask. 1 if not pad. 

        We handle causal masking in addition to pad masking
        '''
        ...
        #x = rearrange(x, "(b s)
        #  h d -> b s (h d)", b=batch_size)

        x_out = self.attn_out(x)

        x = bias_dropout_scale_fn(
            x_out, None, torch.ones_like(x_out), x_skip, self.dropout
        )
...
class DITAR(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    ...
    def forward(self, indices, token_pos, attention_mask):
        """
