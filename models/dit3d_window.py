...
class DiT(nn.Module):
    ...
    def unpatchify_voxels(self, x0):
        ...
        voxels: (N, X, Y, Z, C)      (N, 32, 32, 32, vocab_dim)
        """
        c = self.out_channels
        p = self.patch_size
        ...
        points = x0.reshape(shape=(x0.shape[0], c, x * p, y * p, z * p)) #(N, C, X, Y, Z) 

        # change order to (N, X, Y, Z, C)
        points = torch.permute(points, dims=(0, 2, 3, 4, 1))  
        return points

    def forward(self, x, sigma):
        """
        Forward pass of DiT.
        x: (N, X, Y, Z) voxel grid 
        sigma: (N,) tensor of diffusion timesteps scalars 
        """
        x = torch.unsqueeze(x, dim=1) #(B,1,X,Y,Z)

        x = self.x_embedder(x) 
        x = x + self.pos_embed

        c = nn.functional.silu(self.t_embedder(sigma))                                     
